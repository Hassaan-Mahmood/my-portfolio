...
function makePageInfo(iconTag, descTag, titleTag, ogTag, loaderTitleBox) {
  ...
  var icon =
    ...
    encodeURIComponent(profile.initials) +
...
function makeNavbar(menuBox, logoBox, footerLogo) {
  ...
  for (i = 0; i < navLinks.length; i++) {
    html +=
      "<li><a href='" +
      toSafeText(navLinks[i].link) +
      "'>" +
      toSafeText(navLinks[i].title) +
      "</a></li>";
  }
...
function makeHeroPhoto(pictureBox, chipA, chipB) {
  ...
  if (profile.photo !== "") {
    pictureBox.innerHTML =
      "<img src='" +
      toSafeText(profile.photo) +
      "' alt='Photo of " +
      toSafeText(profile.name) +
      "'>";
  } else {
    pictureBox.innerHTML =
      "<div class='photo-initials'>" + toSafeText(profile.initials) + "</div>";
  }
...
function makeSocialLinks(heroBox, footerBox) {
  ...
  for (i = 0; i < socialLinks.length; i++) {
    html +=
      "<li><a href='" +
      toSafeText(socialLinks[i].link) +
      "' target='_blank' rel='noopener noreferrer'" +
      " aria-label='" +
      toSafeText(socialLinks[i].name) +
      "' title='" +
      toSafeText(socialLinks[i].name) +
      "'>" +
...
function makeAbout(textBox, factsBox, statsBox) {
  ...
  for (i = 0; i < aboutFacts.length; i++) {
    html +=
      "<li><b>" +
      toSafeText(aboutFacts[i].label) +
      "</b>" +
      toSafeText(aboutFacts[i].value) +
      "</li>";
  }
  ...
  for (i = 0; i < stats.length; i++) {
    html +=
      ...
      toSafeText(stats[i].number) +
      "' data-suffix='" +
      toSafeText(stats[i].suffix) +
      "'>0</div>" +
      "<div class='stat-label'>" +
      toSafeText(stats[i].label) +
      "</div>" +
      "</div>";
  }
...
function makeSkills(gridBox) {
  ...
  for (i = 0; i < skillGroups.length; i++) {
    ...
    html +=
      ...
      toSafeText(skillGroups[i].icon) +
      "</div>" +
      "<h3>" +
      toSafeText(skillGroups[i].category) +
      "</h3>" +
      "</div>";
    ...
    for (j = 0; j < skillGroups[i].skills.length; j++) {
      html +=
        ...
        "<div class='skill-top'>" +
        "<span>" +
        toSafeText(skillGroups[i].skills[j].name) +
        "</span>" +
        "<span>" +
        toSafeText(skillGroups[i].skills[j].percent) +
        "%</span>" +
        "</div>" +
        "<div class='bar'>" +
        "<div class='bar-fill' data-percent='" +
        toSafeText(skillGroups[i].skills[j].percent) +
        "'></div>" +
        "</div>" +
        "</div>";
...
function makeFilters(rowBox) {
  ...
  for (i = 0; i < categories.length; i++) {
    if (i === 0) {
      html +=
        "<button type='button' class='filter-btn active' aria-pressed='true' data-category='" +
        toSafeText(categories[i]) +
        "'>" +
        toSafeText(categories[i]) +
        "</button>";
    } else {
      html +=
        "<button type='button' class='filter-btn' aria-pressed='false' data-category='" +
        toSafeText(categories[i]) +
        "'>" +
        toSafeText(categories[i]) +
        "</button>";
...
function makeProjects(gridBox, category) {
  ...
  for (i = 0; i < projects.length; i++) {
    ...
    if (category === "All" || projects[i].category === category) {
      ...
      html +=
        ...
        "<a href='" +
        toSafeText(projects[i].demo) +
        "' target='_blank' rel='noopener noreferrer'" +
        " aria-label='Live demo of " +
        toSafeText(projects[i].title) +
        "'>Live Demo</a>" +
        "<a href='" +
        toSafeText(projects[i].code) +
        "' target='_blank' rel='noopener noreferrer'" +
        " aria-label='Source code of " +
        toSafeText(projects[i].title) +
        "'>Source Code</a>" +
        "</div>";
  ...
  if (shownCount === 0) {
    html =
      "<p class='empty-note'>There are no projects in this category yet.</p>";
  }
...
function makeTimeline(lineBox) {
  ...
  for (i = 0; i < timeline.length; i++) {
    ...
    html +=
      ...
      "<div class='tl-card'>" +
      "<span class='tl-year'>" +
      toSafeText(timeline[i].year) +
      "</span>" +
      "<h3>" +
      toSafeText(timeline[i].title) +
      "</h3>" +
      "<div class='tl-place'>" +
      toSafeText(timeline[i].place) +
      "</div>" +
      "<p>" +
      toSafeText(timeline[i].detail) +
      "</p>" +
      "</div>" +
      "</div>";
  }
...
function makeCertificates(gridBox) {
  ...
  for (i = 0; i < certificates.length; i++) {
    html +=
      ...
      toSafeText(certificates[i].link) +
      "' target='_blank' rel='noopener noreferrer'>" +
      "<span class='cert-year'>" +
      toSafeText(certificates[i].year) +
      "</span>" +
      "<h4>" +
      toSafeText(certificates[i].title) +
      "</h4>" +
      "<p>" +
      toSafeText(certificates[i].issuer) +
      "</p>" +
      "</a>";
  }
...
function makeServices(gridBox) {
  ...
  for (i = 0; i < services.length; i++) {
    html +=
      ...
      "<div class='service-num' aria-hidden='true'>" +
      toSafeText(services[i].icon) +
      "</div>" +
      "<h3>" +
      toSafeText(services[i].title) +
      "</h3>" +
      "<p>" +
      toSafeText(services[i].detail) +
      "</p>" +
      "</div>";
  }
...
function makeContact(cardsBox, noteBox, mailButton) {
  ...
  for (i = 0; i < contactInfo.length; i++) {
    html +=
      "<li><a class='contact-card' href='" +
      toSafeText(contactInfo[i].link) +
      "'>" +
      "<span class='contact-icon' aria-hidden='true'>" +
      toSafeText(contactInfo[i].icon) +
      "</span>" +
      "<span>" +
      "<small>" +
      toSafeText(contactInfo[i].label) +
      "</small>" +
      "<strong>" +
      toSafeText(contactInfo[i].value) +
      "</strong>" +
      "</span>" +
      "</a></li>";
  }
...
function makeFooterColumns(
  ...
  for (i = 0; i < services.length; i++) {
    html +=
      "<li><a href='#services'>" + toSafeText(services[i].title) + "</a></li>";
  }
...
function makeFooterBottom(copyBox, builtBox) {
  ...
  copyBox.innerHTML =
    ...
    year +
    " " +
    toSafeText(footerInfo.owner) +
    ". " +
    toSafeText(footerInfo.rights);
...
//   makeTestimonials(testiGridBox);
  ...
  makeFooterColumns(
    footerLinksHeadingBox,
    footerLinksBox,
    footerServicesHeadingBox,
    footerServicesBox,
    footerContactHeadingBox,
    footerContactBox,
  );
